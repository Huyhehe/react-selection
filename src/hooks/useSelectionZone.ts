...
export type TSelection = { x1: number; y1: number; x2: number; y2: number } | null
...
const checkIntersecting = (selectionBox: NonNullable<TSelection>, itemBox: NonNullable<TSelection>) => {
...
export const useSelectionZone = <T extends TSelectableItem>() => {
  ...
  const handleMouseMove = useCallback(
    ({ e, selection, isSelecting, items, selectionContainerRefCurrent, callback }: HandleMouseMoveCBProps<T>) => {
      if (!selectionContainerRefCurrent || !isSelecting || !selection) return

      const containerRect = selectionContainerRefCurrent.getBoundingClientRect()
      ...
      const selectedItems = items.filter(item => {
        ...
        const { left, top, right, bottom } = itemElement.getBoundingClientRect()
        const itemBox = {
